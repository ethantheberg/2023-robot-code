...
class RobotContainer {
    ...
    val visionSubsystem = VisionSubsystem(swerveSubsystem)
    ...
    private fun VisionTesting (){
        pickAndPlace.defaultCommand = Base(pickAndPlace)

        swerveSubsystem.defaultCommand = StandardDrive(swerveSubsystem,
            { primaryController.leftY * DrivetrainConstants.drivingSpeedScalar / 7.0 },
            { primaryController.leftX * DrivetrainConstants.drivingSpeedScalar / 7.0},
            { primaryController.rightX * DrivetrainConstants.rotationSpeedScalar  / 7.0},
            true,
            true
        )
        ...
        JoystickButton(primaryController, XboxController.Button.kA.value).whileTrue(
            CubeVision(swerveSubsystem, visionSubsystem, primaryController)
        )
    }
...
//            { primaryController.rightX * DrivetrainConstants.rotationSpeedScalar},
        ...
        Trigger { primaryController.rightTriggerAxis > 0.2 }.whileTrue(
            ...
            SequentialCommandGroup(
                ...
                SelectCommand(
                    mapOf(
                        VisionSelector.RETROREFLECTIVE to RetroreflectiveVision(swerveSubsystem, visionSubsystem, primaryController).withTimeout(3.0),
                        VisionSelector.CHUTEVISION to ChuteVision(swerveSubsystem, visionSubsystem, primaryController).withTimeout(6.0),
        ...
        JoystickButton(primaryController, XboxController.Button.kRightBumper.value).whileTrue(
            RetroreflectiveVision(swerveSubsystem,visionSubsystem, primaryController)
        )
        ...
        Trigger {secondaryController.leftTriggerAxis > 0.2} .whileTrue(
            VoltageArm(pickAndPlace, { 3.0 }, { 0.0 }, { 0.0 }, { 0.0 })
        )
